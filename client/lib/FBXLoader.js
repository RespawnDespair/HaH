...
( function() {
	...
	if (!ArrayBuffer.isView) {
		ArrayBuffer.isView = function (arr) {
			return arr.buffer && /\[object (?:(?:.+Array)|DataView)\]/.test(arr.toString());
		}
	}
	...
	Textures.prototype.parse = function ( node, bones ) {
		...
		for ( var n in rawNodes.subNodes ) {

			var tex = ( new Texture() ).parse( rawNodes.subNodes[ n ], node );
			this.add( tex );
